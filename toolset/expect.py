...
class Process:
    def __init__(self, pid, fd):
        ...
        self.fd = fd
        pipe = Pipe(fd)
        self.file = FileWrapper(pipe, pipe, pipe)

    def wait(self):
        self.file.wait()

    def expect(self, *strings):
        return self.file.read_until(*strings)

    def expect_re(self, *patterns):
        return self.file.read_until_regex(*patterns)
    ...
    def kill(self):
        os.kill(self.fd, signal.SIGTERM)
...
def spawn(file, *args):
    try:
        ...
        print("fork failed: %s" % e)
        return
    print("pid: %s, fd: %s" % (pid, fd))
    if pid == pty.CHILD:
        os.execvp(file, [file, *args])
    else:
...
if __name__ == '__main__':
    proc = spawn("sudo", "python3", "app.py")
    res = proc.expect(":")
    proc.send("toybrick\r\n")
